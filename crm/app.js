(function () {
  ...
  const ROLES = {
    admin: { label: 'Администратор', permissions: 'Клиенты, сделки, задачи, сотрудники — полный доступ' },
    manager: { label: 'Менеджер', permissions: 'Клиенты, сделки, задачи — полный доступ' },
    viewer: { label: 'Наблюдатель', permissions: 'Только просмотр клиентов, сделок, задач' }
  };
  ...
  var STAGE_LABELS = { lead: 'Лид', negotiation: 'Переговоры', deal: 'Сделка', done: 'Завершено' };
  ...
  function renderDashboard() {
    ...
    if (recent.length === 0) {
      listEl.innerHTML = '<div class="empty-state">Пока нет клиентов. Добавьте в разделе «Клиенты».</div>';
    } else {
      listEl.innerHTML = recent.map(function (c) {
        var date = c.createdAt ? new Date(c.createdAt).toLocaleDateString('ru-RU') : '—';
  ...
  function formClient(editId) {
    ...
    var phone = c ? (c.phone || '') : '';
    return (
      '<h3>' + (c ? 'Редактировать клиента' : 'Новый клиент') + '</h3>' +
      '<form id="form-client">' +
      '<input type="hidden" name="id" value="' + (editId || '') + '">' +
      '<div class="form-group"><label>Имя / Название</label><input type="text" name="name" value="' + escapeHtml(name) + '" required></div>' +
      '<div class="form-group"><label>Компания</label><input type="text" name="company" value="' + escapeHtml(company) + '"></div>' +
      '<div class="form-group"><label>Email</label><input type="email" name="email" value="' + escapeHtml(email) + '"></div>' +
      '<div class="form-group"><label>Телефон</label><input type="text" name="phone" value="' + escapeHtml(phone) + '"></div>' +
      '<div class="modal-actions">' +
      '<button type="button" class="btn btn--ghost modal-cancel">Отмена</button>' +
      '<button type="submit" class="btn btn--primary">' + (c ? 'Сохранить' : 'Добавить') + '</button>' +
      '</div></form>'
    );
  }
  ...
  function deleteClient(id) {
    if (!confirm('Удалить клиента? Связанные сделки и задачи останутся без привязки.')) return;
  ...
  function renderClients() {
    ...
    if (clients.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" class="empty-state">Нет клиентов. Нажмите «Добавить клиента».</td></tr>';
      return;
    }
    tbody.innerHTML = clients.map(function (c) {
      ...
      return (
        ...
        '<button type="button" class="btn btn--ghost btn--small edit-client" data-id="' + escapeHtml(c.id) + '">Изменить</button>' +
        '<button type="button" class="btn btn--ghost btn--small btn--danger delete-client" data-id="' + escapeHtml(c.id) + '">Удалить</button>' +
        '</td></tr>'
      );
    }).join('');
  ...
  function formDeal(editId) {
    ...
    return (
      '<h3>' + (d ? 'Редактировать сделку' : 'Новая сделка') + '</h3>' +
      '<form id="form-deal">' +
      '<input type="hidden" name="id" value="' + (editId || '') + '">' +
      '<div class="form-group"><label>Название</label><input type="text" name="title" value="' + (d ? escapeHtml(d.title) : '') + '" required></div>' +
      '<div class="form-group"><label>Клиент</label><select name="clientId"><option value="">—</option>' + clientOptions + '</select></div>' +
      '<div class="form-group"><label>Сумма (₽)</label><input type="number" name="amount" value="' + (d ? d.amount : '') + '" min="0" step="1"></div>' +
      '<div class="form-group"><label>Этап</label><select name="stage">' +
      STAGES.map(function (s) {
        var labels = { lead: 'Лид', negotiation: 'Переговоры', deal: 'Сделка', done: 'Завершено' };
        return '<option value="' + s + '"' + (d && d.stage === s ? ' selected' : '') + '>' + (labels[s] || s) + '</option>';
      }).join('') +
      '</select></div>' +
      '<div class="modal-actions">' +
      '<button type="button" class="btn btn--ghost modal-cancel">Отмена</button>' +
      '<button type="submit" class="btn btn--primary">' + (d ? 'Сохранить' : 'Добавить') + '</button>' +
      '</div></form>'
    );
  }
  ...
  function formTask(editId) {
    ...
    return (
      ...
      '<form id="form-task">' +
      '<input type="hidden" name="id" value="' + (editId || '') + '">' +
      '<div class="form-group"><label>Задача</label><input type="text" name="title" value="' + (t ? escapeHtml(t.title) : '') + '" required></div>' +
      '<div class="form-group"><label>Сделка</label><select name="dealId">' + dealOptions + '</select></div>' +
      '<div class="form-group"><label>Срок</label><input type="date" name="dueDate" value="' + due + '"></div>' +
      (t ? '<div class="form-group"><label><input type="checkbox" name="done" ' + (t.done ? 'checked' : '') + '> Выполнено</label></div>' : '') +
      '<div class="modal-actions">' +
      '<button type="button" class="btn btn--ghost modal-cancel">Отмена</button>' +
      '<button type="submit" class="btn btn--primary">' + (t ? 'Сохранить' : 'Добавить') + '</button>' +
      '</div></form>'
    );
  }
  ...
  function renderTasks() {
    ...
    if (sorted.length === 0) {
      listEl.innerHTML = '<div class="empty-state">Нет задач. Нажмите «Добавить задачу».</div>';
      return;
    }
    listEl.innerHTML = sorted.map(function (t) {
      var deal = t.dealId ? deals.find(function (d) { return d.id === t.dealId; }) : null;
      var meta = [];
      if (t.dueDate) meta.push('Срок: ' + new Date(t.dueDate).toLocaleDateString('ru-RU'));
      if (deal) meta.push('Сделка: ' + deal.title);
      return (
        '<div class="task-item' + (t.done ? ' is-done' : '') + '" data-task-id="' + t.id + '">' +
        '<input type="checkbox" class="task-item__checkbox" ' + (t.done ? 'checked' : '') + ' aria-label="Выполнено">' +
        '<div class="task-item__body">' +
        '<div class="task-item__title">' + escapeHtml(t.title) + '</div>' +
        '<div class="task-item__meta">' + (meta.join(' · ') || '') + '</div></div>' +
        '<div class="task-item__actions">' +
        '<button type="button" class="btn btn--ghost btn--small edit-task" data-id="' + t.id + '">Изменить</button>' +
  ...
  function formEmployee(editId) {
    ...
    return (
      '<h3>' + (emp ? 'Редактировать сотрудника' : 'Новый сотрудник') + '</h3>' +
      '<form id="form-employee">' +
      '<input type="hidden" name="id" value="' + (editId || '') + '">' +
      '<div class="form-group"><label>Имя</label><input type="text" name="name" value="' + (emp ? escapeHtml(emp.name) : '') + '" required></div>' +
      '<div class="form-group"><label>Email</label><input type="email" name="email" value="' + (emp ? escapeHtml(emp.email) : '') + '" required></div>' +
      '<div class="form-group"><label>Роль и права доступа</label><select name="role">' + roleOptions + '</select></div>' +
      '<div class="modal-actions">' +
      '<button type="button" class="btn btn--ghost modal-cancel">Отмена</button>' +
  ...
  function renderEmployees() {
    ...
    if (employees.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" class="empty-state">Нет сотрудников. Нажмите «Добавить сотрудника».</td></tr>';
      return;
    }
    tbody.innerHTML = employees.map(function (emp) {
      ...
      return (
        ...
        '<td class="row-actions">' +
        '<button type="button" class="btn btn--ghost btn--small edit-employee" data-id="' + escapeHtml(emp.id) + '">Изменить</button>' +
        '<button type="button" class="btn btn--ghost btn--small btn--danger delete-employee" data-id="' + escapeHtml(emp.id) + '">Удалить</button>' +
        '</td></tr>'
      );
    }).join('');
